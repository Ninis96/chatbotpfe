...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    /*var greeting = new RegExp(/^(yo|salut|hola|howdy|hi|hey|hello)/i);
    ...
    if (messageText.match(hashtag)) {
      ...
      recastClient.analyseText(messageText).then(function(response) {
        ...
        if (intent == 'greetings') {
          ...
          sendTextMessage(senderID, "Ciao, à bientôt!")
        } else if (intent == 'trends') {
          getTrends(senderID);
        } else if (intent == 'gif') {
          sendGifMessage(senderID);
        } else if (intent == 'presentation') {
          sendTextMessage(senderID, "Je m'appelle Twittee, avec moi tu peux savoir les tweets tendances, rechercher des hashtags et si tu veux voir des gifs drôles il suffit de demander! :)")
        } else {
          sendTextMessage(senderID, messageText);
        }
  ...
  var messageData = {
    ...
    message: {
      attachment: {
        type: "image",
        payload: {
          url: SERVER_URL + "/assets/cats.gif"
        }
      }
    }
  };
...
function sendQuickReply(recipientId, list) {
  ...
  var messageData = {
    ...
    message: {
      text: "Voici les hashtags tendances du jour..",
      metadata: "DEVELOPER_DEFINED_METADATA",
...
function getTweets(senderID, trend) {
  twitterClient.get('search/tweets', {q: trend}, function(error, body, response) {
    if (error) {
      ...
      var illustratedTweets = body.statuses.filter(t => { return t.entities.media }).slice(0,5);
      if (illustratedTweets.length > 0) {
        ...
        sendTextMessage(senderID, "Désolé, je n'arrive pas à trouver les tweets sans images..😢")
      }
    }
  });
}
 ...
 *
 */
function getGoogleSearch(senderId, message) {
  var messageData = {
    recipient: {
      id: recipientId
    },
    message: {
      attachment: {
        ...
        payload: {
          ...
          buttons:[{
            type: "web_url",
            url: "https://www.google.fr/search?q=" + selected_trend,
            title: "Search trend on Google"
          }]
        }
      }
    }
